...
async function definition(phrase) {
    ...
    if (!result.isSuggestions && !result.isCached) {
        await DatabaseService.insertDefinition(phrase, resp).catch(e => Logger.debug('Error saving to cache', e));
    }
...
async function thesaurus(phrase) {
    ...
    if (!resp.cached)
        await DatabaseService.insertThesaurus(phrase, resp).catch(e => Logger.debug('Error saving to cache', e));
